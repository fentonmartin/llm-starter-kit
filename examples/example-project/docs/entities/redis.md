...
Single primary with one replica ([[260710-ops-runbook]], "Where sessions live"). Database 0.

## What it stores

Session records under the key prefix `sess:`, keyed by the opaque session identifier. Each value
is a serialized map of user id, issue time, and originating IP ([[260415-auth-spec]], §3).

## Configuration

| Setting | Value | Source |
|---|---|---|
| `SESSION_TTL` | `86400` seconds — 24 hours, raised from 1800 on 2026-06-02 | [[260710-ops-runbook]] |
| `SESSION_IDLE` | `900` seconds — 15 minutes | [[260710-ops-runbook]] |
| `REDIS_MAXMEMORY_POLICY` | `noeviction` | [[260710-ops-runbook]] |
...
- Draining the primary logs out every user, so restarts are scheduled outside business hours
  ([[260710-ops-runbook]], "Restart procedure").
- Eviction is disabled deliberately. Under memory pressure Redis refuses writes rather than
  dropping sessions ([[260710-ops-runbook]], "Configuration in production").
